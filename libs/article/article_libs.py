...
ct_dict[modelname].by_name(name)
'''
...
def del_category_tag_lib(self, c_uuid, t_uuid):
    ...
    if c_uuid is not None:
        ...
        if category is None:
            ...
            return {'status': False, 'msg': '分类不存在'}
        if category.articles:
            flash(self, '分类下有文章请先删除文章', 'error')
            return {'status': False, 'msg': '分类下有文章， 请先删除文章'}
        self.db.delete(category)
        flash(self, '分类删除成功', 'success')
        self.db.commit()
        return {'status': True, 'msg': '分类删除成功'}
    if t_uuid is not None:
        ...
        self.db.commit()
        return {'status': True, 'msg': '标签删除成功'}
    flash(self, '请输入标签或分类', 'error')
    return {'status': False, 'msg': '请输入标签或分类'}

def article_content_libs(self, article_id):
    """05文章详情"""
    if article_id is None:
        return {'status': False, 'msg': '缺少文章ID'}
    article= Article.by_id(article_id)
    if article is None:
        return {'status': False, 'msg': '文章更不存在'}
    article.readnum +=1
    self.db.add(article)
    self.db.commit()
...
def add_like_lib(self, article_id):
    ...
    if self.current_user in article.user_likes:
        ...
        self.db.add(article)
        self.db.commit()
        return {'status': True, 'msg': '点赞已经取消'}
        # return {'status': False,'msg': '您已经点过赞了~~!'}
